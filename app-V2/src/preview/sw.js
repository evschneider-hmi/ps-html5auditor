...
(() => {
  ...
  const noteMiss = async (url) => {
    ...
    try {
      ...
      for (const client of clients) {
        client.postMessage({ type: 'CM360_SW_FETCH_MISS', url });
      }
    } catch (error) {
      // eslint-disable-next-line no-console
      console.warn('CM360_SW_FETCH_MISS failed', error);
    }
  };
  ...
  self.addEventListener('message', (event) => {
    ...
    console.log('[SW] Received message:', data?.type, '| has source:', !!event.source);
    ...
    if (data.type === 'CM360_BUNDLE_ENTRIES') {
      ...
      console.log('[SW] Stored entries:', entries.size, '| posting acknowledgment');
      // Acknowledge that entries have been stored
      // Use clients.matchAll with includeUncontrolled to find the iframe
      self.clients.matchAll({ includeUncontrolled: true, type: 'window' }).then((clients) => {
        console.log('[SW] Found clients:', clients.length);
        clients.forEach((client) => {
          console.log('[SW] Posting to client:', client.url);
          client.postMessage({
            type: 'CM360_ENTRIES_STORED',
            count: entries.size,
          });
        });
      }).catch((error) => {
        console.error('[SW] clients.matchAll failed:', error);
      });
    }
  });
