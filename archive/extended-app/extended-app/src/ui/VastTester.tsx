...
export const VastTester: React.FC = () => {
  const vastAutoPayload = useExtStore(s => s.vastAutoPayload);
  ...
  const [autoRows, setAutoRows] = useState<Array<{ i:number; url:string; label:string }>>([]);
  ...
  // When we land here with a ZIP from the HTML5 tab, parse any sheet files to collect VAST URLs
  useEffect(()=>{
    let cancelled = false;
    (async()=>{
      try {
        if (!vastAutoPayload || !(vastAutoPayload instanceof Uint8Array)) return;
        const zip = await JSZip.loadAsync(vastAutoPayload);
        const entries = Object.keys(zip.files);
        const rows: Array<{ i:number; url:string; label:string }> = [];
        let idx = 1;
        for (const path of entries) {
          const entry = zip.file(path);
          if (!entry || entry.dir) continue;
          const norm = path.replace(/^\/+/, '');
          if (norm.startsWith('__MACOSX/') || /(^|\/)\._/.test(norm)) continue;
          const isCsv = /\.csv$/i.test(norm);
          if (!isCsv && !/\.(xlsx|xlsm|xlsb|xls)$/i.test(norm)) continue;
          const data = isCsv ? await entry.async('string') : await entry.async('arraybuffer');
          const wb = XLSX.read(data, { type: isCsv ? 'string' : 'array', raw:false, cellDates:true });
          wb.SheetNames.forEach(sheetName => {
            ...
            const hdr = (table[0]||[]) as any[];
            const map = new Map<string, number>();
            hdr.forEach((c, i) => { const k = String(c||'').toLowerCase().trim(); if (k) map.set(k, i); });
            function pick(row:any[], keys:string[]): string | undefined {
              for (const k of keys){ const i = map.get(k); if (i===undefined) continue; const v = (row[i]||'').toString().trim(); if (v) return v; }
              return undefined;
            }
            for (let r=1; r<table.length; r++){
              const row = table[r] as any[]; if (!row) continue;
              let vast = pick(row, ['vast url','vast','vasturl']);
              if (!vast){
                const textCells = row.map(v=>String(v||''));
                vast = textCells.find(v => /https?:\/\//i.test(v) && /vast/i.test(v));
              }
              if (vast){
                rows.push({ i: idx++, url: vast, label: `${norm} — ${sheetName} #${r+1}` });
              }
            }
          });
        }
        if (!cancelled) setAutoRows(rows);
        // Keep payload so user can switch around tabs without re-uploading
      } catch (e:any) {
        if (!cancelled) setErrors(prev => [...prev, `Failed to parse embedded sheets: ${e.message||String(e)}`]);
      }
    })();
    ...
    <div>
      {autoRows.length>0 && (
        <div className="panel" style={{ padding:8, marginBottom:8 }}>
          <div style={{ fontSize:12, fontWeight:700, marginBottom:6 }}>VAST URLs found in your uploaded ZIP</div>
          <div style={{ overflowX:'auto' }}>
            <table className="table">
              <thead><tr><th>#</th><th>VAST URL</th><th>Source</th></tr></thead>
              <tbody>
                {autoRows.map(r => (
                  <tr key={r.i} role="button" tabIndex={0} onClick={()=>{ setMode('url'); setTagUrl(r.url); setTimeout(()=>{ void loadTag(); }, 0); }} onKeyDown={(e)=>{ if (e.key==='Enter' || e.key===' '){ e.preventDefault(); setMode('url'); setTagUrl(r.url); setTimeout(()=>{ void loadTag(); }, 0); } }} style={{ cursor:'pointer' }}>
                    <td>{r.i}</td>
                    <td title={r.url} style={{ maxWidth:520, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{r.url}</td>
                    <td title={r.label} style={{ maxWidth:520, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{r.label}</td>
                  </tr>
                ))}
                {autoRows.length===0 && (<tr><td colSpan={3} style={{ fontStyle:'italic', color:'#6b7280' }}>No VAST URLs detected</td></tr>)}
              </tbody>
            </table>
          </div>
        </div>
      ...
      <div className="tabs" style={{ marginTop:0, marginBottom:8 }}>
        <button onClick={()=>setMode('url')} className={`tab ${mode==='url' ? 'active' : ''}`}>Tag URL</button>
        <button onClick={()=>setMode('xml')} className={`tab ${mode==='xml' ? 'active' : ''}`}>VAST XML</button>
      </div>
      {mode==='url' ? (
        <input value={tagUrl} onChange={e=>setTagUrl(e.target.value)} placeholder="https://example.com/path/to/vast.xml" style={{ width:'100%', padding:8, border:'1px solid var(--border)', borderRadius:6, fontFamily:'monospace', fontSize:12, background:'var(--surface)', color:'var(--text)' }} />
      ) : (
  ...
  <div style={{ marginTop:8 }}><button onClick={loadTag} className="btn primary">Load</button></div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginTop:12 }}>
        <div>
          <div style={{ fontSize:12, fontWeight:700 }}>Errors</div>
          <ul style={{ fontFamily:'monospace', fontSize:11, minHeight:80, background:'var(--surface-2)', color:'var(--text)', padding:6, borderRadius:6, border:'1px solid var(--border)' }}>{errors.map((e,i)=>(<li key={i}>{e}</li>))}</ul>
        </div>
        <div>
          <div style={{ fontSize:12, fontWeight:700 }}>Info</div>
          <ul style={{ fontFamily:'monospace', fontSize:11, minHeight:80, background:'var(--surface-2)', color:'var(--text)', padding:6, borderRadius:6, border:'1px solid var(--border)' }}>{info.map((e,i)=>(<li key={i}>{e}</li>))}</ul>
        </div>
      </div>
      ...
      {mediaUrl && (
        <div style={{ marginTop:16 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div>
              <video ref={videoRef} src={mediaUrl} controls muted playsInline style={{ width:'100%', border:'1px solid var(--border)', borderRadius:6 }} onCanPlay={()=>{ /* allow auto play */ try{ videoRef.current?.play().catch(()=>{});}catch{} }} />
              {clickThrough && (
                <div style={{ marginTop:6 }}>
                  <button onClick={()=>{ setClicked(true); log('click'); fireEvent('click'); window.open(clickThrough, '_blank', 'noopener'); }} className="btn primary">Open ClickThrough</button>
                </div>
              )}
            </div>
            <div>
              ...
              <div style={{ maxHeight:200, overflow:'auto', border:'1px solid var(--border)', borderRadius:6, background:'var(--surface)' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                  <thead><tr style={{ background: 'var(--table-head)' }}><th style={th}>Time</th><th style={th}>Event</th></tr></thead>
                  <tbody>
                    {timeline.map((row, i)=>(<tr key={i} style={{ borderTop:'1px solid var(--border)' }}><td style={td}>{row.time}</td><td style={td}>{row.label}</td></tr>))}
              ...
              <div style={{ maxHeight:220, overflow:'auto', border:'1px solid var(--border)', borderRadius:6, background:'var(--surface)' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                  <thead><tr style={{ background: 'var(--table-head)' }}><th style={th}>Event</th><th style={th}>URL</th><th style={th}>Fired At</th><th style={th}>Status</th></tr></thead>
                  <tbody>
                    ...
                    {Object.entries(trackers).flatMap(([ev, arr])=> arr.map((it,i)=> (
                      <tr key={ev+'-'+i} style={{ borderTop:'1px solid var(--border)' }}>
                        <td style={td}>{ev}</td>
      ...
      {rawXml && (
        <div style={{ marginTop:16 }}>
          <details open>
            ...
            <pre style={{ whiteSpace:'pre-wrap', wordBreak:'break-word', fontFamily:'monospace', fontSize:11, lineHeight:1.3, background:'var(--surface-2)', color:'var(--text)', padding:8, borderRadius:6, border:'1px solid var(--border)' }}>{rawXml}</pre>
          </details>
        </div>
      )}
    </div>
...
function tabBtn(active:boolean): React.CSSProperties { return { padding:'6px 10px', borderRadius:6, border:'1px solid var(--btn-border)', background: active ? 'var(--tab-active-bg)' : 'var(--btn-bg)', color: active ? 'var(--text)' : 'var(--text-2)', fontSize:12, fontWeight:600 }; }

const Step: React.FC<{ label:string; done?: boolean; hint?: string }> = ({ label, done, hint }) => (
  <div title={hint} style={{ padding:'6px 8px', borderRadius:6, background: done ? 'var(--ok)' : 'var(--btn-bg)', color: done ? '#fff' : 'var(--text)', textAlign:'center', fontSize:12, fontWeight:700, border:'1px solid var(--btn-border)' }}>{label}</div>
);
