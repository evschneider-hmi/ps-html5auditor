...
export async function buildExtendedFindings(bundle: ZipBundle, partial: BundleResult, settings: any): Promise<Finding[]> {
  ...
  // Filename Contains Dimensions (required): verify size token like 300x250 or 300 X 250 appears in bundle/primary name
  {
    const size = partial.adSize;
    if (size && typeof size.width==='number' && typeof size.height==='number') {
      const w = String(size.width);
      const h = String(size.height);
      // allow optional spaces around x/X; require boundaries not surrounded by digits
      const token = `${w}x${h}`;
      const re = new RegExp(`(?<!\\d)${w}\\s*[xX]\\s*${h}(?!\\d)`);
      const names: { label: string; value: string }[] = [];
      try { names.push({ label: 'ZIP name', value: (partial as any).bundleName || '' }); } catch {}
      try { const base = (primary||'').split('/').pop()||primary||''; names.push({ label: 'Primary HTML', value: base }); } catch {}
      const offenders: any[] = [];
      let ok = false;
      for (const n of names) {
        if (n.value && re.test(n.value)) { ok = true; } else if (n.value) { offenders.push({ path: n.value }); }
      }
      const severity = ok ? 'PASS' : 'FAIL';
      const examples = `${w}x${h} or ${w} X ${h}`;
      out.push({
        id: 'nameDimensions',
        title: 'Filename Contains Dimensions',
        severity: severity as any,
        messages: ok ? [`Found size token: ${token}`] : [`Expected size token in name: ${examples}`],
        offenders,
      });
    }
  }
  ...
  // Memory / CPU Heuristics
  {
    ...
    const hasMem = typeof mem === 'number' && isFinite(mem);
    const hasCpu = typeof cpu === 'number' && isFinite(cpu);
    const memMsg = hasMem ? `Heap ~${mem.toFixed(1)} MB` : 'Heap: not available';
    const cpuMsg = hasCpu ? `CPU jitter score ${cpu.toFixed(2)}` : 'CPU: not available';
    // Advisory thresholds (optional check): warn only on egregious values; pass otherwise.
    // Treat missing signals as WARN (advisory) rather than FAIL.
    let sev: 'PASS'|'WARN'|'FAIL' = 'PASS';
    if (!hasMem && !hasCpu) {
      sev = 'WARN';
    } else {
      const memWarn = hasMem && mem >= 128; // MB
      const cpuWarn = hasCpu && cpu >= 0.50; // 0..1 (higher is worse)
      sev = (memWarn || cpuWarn) ? 'WARN' : 'PASS';
    }
    out.push({ id:'perfHeuristics', title:'CPU/Memory Heuristics', severity: sev as any, messages: [memMsg, cpuMsg, 'Advisory: measured in preview, varies by device'], offenders: [] });
  }
  ...
  // Creative Border â€” detect CSS border: ... or four 1px edge lines (common in GWD)
  {
    ...
    const cssHasBorder = cssTexts.some(t => /\bborder(?:-top|-right|-bottom|-left|)\s*:\s*\d+px\s+(solid|dashed|double)/i.test(t));
    ...
    try {
      ...
      function isPx1to4(val?: string){ if(!val) return false; const m = /^([1-4])(px)?$/.exec(val); return !!m; }
      ...
      for (const el of nodes) {
        ...
        // top line
        if (isZero(st['top']) && isZero(st['left']) && isFull(st['width']) && isPx1to4(st['height']) && isVisibleColor(bg)) { top = true; offenders.push({ path: '(inline)', detail: `top line ${st['height']||''} via absolute div` }); continue; }
        // bottom line
        if (isZero(st['bottom']) && isZero(st['left']) && isFull(st['width']) && isPx1to4(st['height']) && isVisibleColor(bg)) { bottom = true; offenders.push({ path: '(inline)', detail: `bottom line ${st['height']||''} via absolute div` }); continue; }
        // left line
        if (isZero(st['left']) && isZero(st['top']) && isFull(st['height']) && isPx1to4(st['width']) && isVisibleColor(bg)) { left = true; offenders.push({ path: '(inline)', detail: `left line ${st['width']||''} via absolute div` }); continue; }
        // right line
        if (isZero(st['right']) && isZero(st['top']) && isFull(st['height']) && isPx1to4(st['width']) && isVisibleColor(bg)) { right = true; offenders.push({ path: '(inline)', detail: `right line ${st['width']||''} via absolute div` }); continue; }
      }
